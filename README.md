Проект пишется на микросервисной архитектуре, однако на данный момент ещё не завершен, поэтому пока что используется стандартный Layout из шаблона, который будет меняться. Также какие-то страницы ещё не стилизованы.
Проект включает в себя процесс регистрации как работника или работодателя на основе JWT токенов, а также подтверждение почты отправкой письма на неё.
