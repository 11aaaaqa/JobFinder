...
{
    public class EmployerRemovedFromCompanyKafkaConsumer(IConfiguration configuration, IServiceScopeFactory scopeFactory) : BackgroundService
    {
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            const string topicName = "employer-removed-from-company-topic";
            ...
            {
                ConsumeResult<Null, string> consumerResult = new();
                try
                {
                ...
                var model = JsonSerializer.Deserialize<EmployerRemovedFromCompanyModel>(consumerResult.Message.Value);
